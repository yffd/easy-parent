...
public class CustomPermissionFilter extends AccessControlFilter {
	private static final Logger LOG = LoggerFactory.getLogger(CustomPermissionFilter.class);
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)
			throws Exception {
		if(LOG.isInfoEnabled()) LOG.info("========= custom filter permission =========");
		// 未指定权限，拒绝通过
		if(null==mappedValue) {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		if(LOG.isInfoEnabled()) LOG.info("========= custom filter permission error : " + ShiroConstants.MSG_KEY_DESC_100005);
		// 输出信息
