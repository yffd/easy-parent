 ...
 */
@Service
public class UupmSecRelationRoleUserService extends UupmBaseService<UupmSecRelationRoleUser> {

	@Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public int saveRoleResource(String ttCode, String userCode, List<String> roleCodes, LoginInfo loginInfo) {
		if (EasyStringCheckUtils.isEmpty(ttCode) || EasyStringCheckUtils.isEmpty(userCode)) throw CommonBizException.BIZ_PARAMS_IS_EMPTY();
		if (null == roleCodes || roleCodes.size() == 0) return 0;
		// 先删除已有的关联
		UupmSecRelationRoleUser relationEntity = new UupmSecRelationRoleUser();
		relationEntity.setTtCode(ttCode);
		relationEntity.setUserCode(userCode);
		this.delete(relationEntity, null);
		// 添加关联
		List<UupmSecRelationRoleUser> entityList = new ArrayList<>();
		for (String roleCode : roleCodes) {
			UupmSecRelationRoleUser entity = new UupmSecRelationRoleUser();
